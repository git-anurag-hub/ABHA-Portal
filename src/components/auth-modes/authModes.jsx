...
const AuthModes = (props) => {
  ...
  const [ndhmDetails, setNdhmDetails] = [
    props.ndhmDetails,
    props.setNdhmDetails,
  ];
  ...
  async function authenticate() {
    ...
    if (!props.isHealthNumberNotLinked) {
      ...
      if (checkIfAuthModeSupported()) {
        ...
        setErrorHealthId(
          "The selected Authentication Mode is currently not supported!"
        );
        setShowError(true);
      }
    }
  ...
  function checkIfAuthModeSupported() {
    if (
      (selectedAuthMode === "DEMOGRAPHICS" && !isDemoAuthEnabled) ||
  ...
  return (
    <div>
      {!checkIfNotNull(ndhmDetails) && (
        <div className="auth-modes">
          ...
          <div className="auth-modes-select-btn">
            ...
            <button
              type="button"
              disabled={showOtpField || isDirectAuth}
      ...
      {!isDirectAuth && showOtpField && (
        <OtpVerification
          id={id}
          selectedAuthMode={selectedAuthMode}
          ndhmDetails={ndhmDetails}
          setNdhmDetails={setNdhmDetails}
          isHealthNumberNotLinked={props.isHealthNumberNotLinked}
        />
      )}
    </div>
  );
