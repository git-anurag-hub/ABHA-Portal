...
const VerifyHealthId = () => {
  ...
  const [isVerifyABHAThroughFetchModes, setIsVerifyABHAThroughFetchModes] =
    useState(false);
  const [isVerifyThroughABHASerice, setIsVerifyThroughABHASerice] =
    useState(false);
  ...
  async function verifyHealthId() {
    ...
    const healthIdStatus =
      matchingPatientId.Error !== undefined
        ? await getHealthIdStatus(matchingPatientId.patientUuid)
        : false;
    if (matchingPatientId.Error === undefined) {
      if (healthIdStatus === true) setHealthIdIsVoided(true);
  ...
  async function searchByHealthId() {
    ...
    if (response.data !== undefined) {
      ...
      if (response.data.status === "ACTIVE") {
        ...
        setAuthModes(
          response.data.authMethods !== undefined
            ? response.data.authMethods.filter((e) =>
                supportedHealthIdAuthModes.includes(e)
              )
            : []
        );
      } else {
      ...
      if (
        enableHealthIdVerify &&
        IsValidPHRAddress(id) &&
        response.details[0].code === "HIS-1008"
      ) {
  ...
  return (
    <div>
        ...
        <div>
          <div className="verify-health-id">
            <label htmlFor="healthId" className="label">
              Enter ABHA Number/ABHA Address:{" "}
            </label>
            <div className="verify-health-id-input-btn">
              ...
              <button
                ...
                disabled={showAuthModes || checkIfNotNull(ndhmDetails)}
              >
                Verify
              </button>
          ...
          <div className="alternative-text">OR</div>
          <div className="qr-code-scanner">
            <button
              name="scan-btn"
              type="button"
              onClick={() => setScanningStatus(!scanningStatus)}
              disabled={showAuthModes || checkIfNotNull(ndhmDetails)}
            >
              Scan Patient's QR code <span id="cam-icon">Web cam</span>
            </button>
            {scanningStatus && (
              <QrReader
                delay={10}
          ...
          {matchingPatientFound && (
            <div
              className="patient-existed"
              onClick={redirectToPatientDashboard}
            >
              Matching record with Health ID/PHR Address found
            </div>
          )}
          {healthIdIsVoided && (
            <div className="id-deactivated">Health ID is deactivated</div>
          )}
          {!showAuthModes && isHealthNumberNotLinked && (
            <div>
              <div className="note health-id">
                Health Id doesn't have health Number linked. Click on proceed to
                authenticate with only healthId or you can create new ABHA
                Number
              </div>
              <div className="proceed-button">
                <button name="proceed-btn" type="button" onClick={verify}>
                  Proceed
                </button>
              </div>
          ...
          {showAuthModes && (
            <AuthModes
              id={id}
              authModes={authModes}
              ndhmDetails={ndhmDetails}
              setNdhmDetails={setNdhmDetails}
              setIsDemoAuth={setIsDemoAuth}
              isHealthNumberNotLinked={isHealthNumberNotLinked}
            />
          )}
        </div>
      )}
      ...
      {isVerifyThroughABHASerice &&
        checkIfNotNull(ndhmDetails) &&
        ndhmDetails.id === undefined && (
          <CreateHealthId
            ndhmDetails={ndhmDetails}
            setNdhmDetails={setNdhmDetails}
            setIsHealthIdCreated={setIsHealthIdCreated}
          />
        )}
      {!matchingPatientFound &&
        !healthIdIsVoided &&
        ...
        (ndhmDetails.id !== undefined ||
          isHealthIdCreated ||
          isVerifyABHAThroughFetchModes) && (
          <PatientDetails
            ndhmDetails={ndhmDetails}
            id={id}
            setBack={setBack}
            isVerifyABHAThroughFetchModes={
              isVerifyABHAThroughFetchModes || !isHealthIdCreated
            }
          />
        )}
    </div>
