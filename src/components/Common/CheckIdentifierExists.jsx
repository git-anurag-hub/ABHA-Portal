  ...
  async function checkIfAlreadyExistingIdentifier(id) {
    ...
    if (id !== "") {
      ...
      if (
        ...
        const healthIdStatus = await getHealthIdStatus(
          matchingPatientId.patientUuid
        );
        if (healthIdStatus) {
    ...
    <div>
      {matchingPatientFound && (
        <div className="matched-patient-info">
          Matching record with {props.id} found. Please proceed to update the
          record
        </div>
      )}
      {healthIdIsVoided && (
        <div className="id-deactivated">{props.id} is deactivated</div>
      )}
