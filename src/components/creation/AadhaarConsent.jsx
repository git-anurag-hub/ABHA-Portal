...
const AadhaarConsent = (props) => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      aadhaarConsent.filter((e) => e === true).length === aadhaarConsent.length
    ) {
      setError(false);
    ...
    <div>
        ...
        {AadhaarConsents.map((consent, index) => {
          return (
            <div>
              ...
              {index > 3 && (
                <div className="consent-input sub-consent-input">
                  <input
                    type="checkbox"
                    id={index}
                    checked={aadhaarConsent[index]}
      ...
      {error && (
        <p className="error-msg">
          *Please select all the above checkboxes and enter beneficiary name to
          proceed
        </p>
      )}
    </div>
