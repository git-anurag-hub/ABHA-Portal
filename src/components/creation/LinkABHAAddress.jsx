...
const LinkABHAAddress = (props) => {
  ...
  const handleClickOutside = (event) => {
    if (
      refEls.current &&
      !Object.values(Object.values(refEls.current)).includes(event.target)
    ) {
  ...
  let phrAddressList =
    ...
    patient.phrAddress.length > 0 &&
    patient.phrAddress.map((item, i) => {
      return (
        <div>
          <button
            ref={(element) => (refEls.current[i] = element)}
            onClick={() => {
              setAbhaAddress(patient.phrAddress[i]);
              setIsAbhaSelected(true);
            }}
            className={abhaAddress === item ? "active" : "abha-list"}
          >
            {item}
          </button>
        </div>
  ...
  function mapPatient() {
    props.mappedPatient.id = abhaAddressCreated
      ? newAbhaAddress.concat(cmSuffix)
      : abhaAddress;
  }
  ...
  return (
    <div>
        ...
        <div>
          {patient.phrAddress === undefined && (
            <div className="no-abha-address">
              <p className="note">
                No ABHA address found linked to the ABHA number
              </p>
              <p className="note">
                {isLinkingEnabled && (
                  <>
                    {" "}
                    Please proceed with linking the ABHA address that is already
                    mapped to the mobile number or email, or{" "}
                  </>
                )}
                create a new ABHA address
              </p>
            </div>
          )}
          {patient.phrAddress !== undefined && (
            <div>
              <div className="choose-abha-address">
                <div className="abha-list-label">
                  <label htmlFor="abha-address">
                    Choose from existing ABHA Addresses linked to ABHA Number.
                  </label>
                </div>
                {phrAddressList}
              </div>
              {isAbhaSelected && (
                <CheckIdentifierExists
                  id={abhaAddress}
                  setHealthIdIsVoided={setHealthIdIsVoided}
                  setMatchingPatientUuid={setMatchingPatientUuid}
                />
              )}
              {abhaAddress !== "" && (
                <div className="center">
                  <button
                    type="button"
                    disabled={
                      healthIdIsVoided || !isAbhaSelected ? true : false
                    }
                    className="proceed"
                    onClick={onProceed}
                  >
          ...
          {isLinkingEnabled && (
            <div className="left-button">
              <button
                type="button"
                disabled={isAbhaSelected ? true : false}
                className="proceed"
                title="Link exisiting ABHA Address linked to Mobile/Email"
                onClick={gotoLink}
              >
                Link ABHA Address
              </button>
          ...
          <div
            className={
              isLinkingEnabled ? "right-button" : "create-new-abhaAddress"
            }
          >
            <button
              type="button"
      ...
      {!proceed && createNewABHA && (
        <CreateABHAAddress
          setBack={setBack}
          newAbhaAddress={newAbhaAddress}
          setNewAbhaAddress={setNewAbhaAddress}
          setABHAAddressCreated={setABHAAddressCreated}
        />
      )}
      {link && (
        <VerifyMobileEmail
          patient={patient}
          setBack={setBack}
          mappedPatient={props.mappedPatient}
        />
      )}
      {proceed && (
        <PatientDetails
          ndhmDetails={props.mappedPatient}
          setBack={!abhaAddressCreated ? setBack : undefined}
        />
      )}
    </div>
  );
