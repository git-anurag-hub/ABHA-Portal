...
import {
  fetchPatientDetailsFromBahmni,
  saveDemographics,
  fetchPatientFromBahmniWithUuid,
...
const PatientDetails = (props) => {
  ...
  async function fetchPatientDetailsFromBahmniWithUuid() {
    const response = [];
    const existingPatient = await fetchPatientFromBahmniWithUuid(
      ndhmDetails.uuid
    );
    if (existingPatient.Error === undefined && existingPatient !== null) {
      response.push(existingPatient);
      const parsedPatients = response.map((patient) => {
  ...
  function getPhoneNumber() {
    if (
      ndhmDetails?.identifiers !== undefined &&
      ndhmDetails?.identifiers.length > 0
    ) {
  ...
  function save(isConfirmSelected) {
    var healthNumber = new Identifier(
      new Type("ABHA Number"),
      getHealthNumber(ndhmDetails)
    );
    var healthId = new Identifier(
      new Type("ABHA Address"),
      ndhmDetails?.id || "-"
    );
    ...
    if (ndhmDetails.name) {
      ...
      var familyName =
        name?.length === 3 ? name[2] : name?.length > 1 ? name[1] : "";
    ...
    var telecom = getPhoneNumber()
      ? new Telecom("phone", getPhoneNumber())
      : null;
    var address = new Address(
      ndhmDetails.address?.line,
      ndhmDetails.address?.city,
      ndhmDetails.address?.district,
      ndhmDetails.address?.state,
      ndhmDetails.address?.pincode,
      "IN"
    );
    ...
    let patient = new FhirPatient(
      id,
      [healthId, healthNumber],
      names,
      gender,
      dob,
      ndhmDetails?.isBirthDateEstimated,
      address,
      telecom,
      ""
    );
    ...
    return patients.map((patient, i) => {
      return (
        <button
          ...
          disabled={checkIfNotNull(selectedPatient)}
          className="matching-patient"
        >
  ...
  return (
    <div className="matching-patients">
      <div className={checkIfNotNull(selectedPatient) ? "greyed-out" : ""}>
        <b>ABDM Record: </b>
        <PatientInfo patient={ndhmDetails} />
        <br />
        ...
        {patients.length > 0 && ndhmDetails.uuid == undefined && (
          <div>
            ...
            <div className="matched-record-note">
              <p>
                Select the appropriate <b>matched</b> Bahmni record to update
                the data for that patient in the system, or click on{" "}
                <b>Create New Record</b>, if you want to create a fresh patient
                record
              </p>
            </div>
        ...
        {patients.length > 0 && ndhmDetails.uuid != undefined && (
          <div>
            <div className="matched-record-note">
              <p>
                The Bahmni record below will be updated with the ABDM details
                above. Please click on the record to proceed.
              </p>
            </div>
            {prepareMatchingPatientsList()}
          </div>
        ...
        <div className="create-confirm-btns">
          {props.setBack !== undefined && (
            <button onClick={() => props.setBack(true)}>back</button>
          )}
          {(ndhmDetails.uuid == undefined || ndhmDetails.uuid == "") && (
            <button onClick={updateRecord}> Create New Record </button>
          )}
        </div>
      </div>
      {checkIfNotNull(selectedPatient) && (
        <ConfirmPopup
          selectedPatient={selectedPatient}
          close={() => setSelectedPatient({})}
...
export const getHealthNumber = (ndhmDetails) => {
  if (ndhmDetails?.healthIdNumber) return ndhmDetails?.healthIdNumber;
  let healthNumber;
