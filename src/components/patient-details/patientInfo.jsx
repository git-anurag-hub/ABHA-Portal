...
const PatientInfo = (props) => {
  ...
  function getCustomAddress(patient) {
    ...
    for (var key in patient.address) {
      if (patient.address[key] && typeof patient.address[key] === "object") {
        patient.address[key] = patient.address[key].filter(
  ...
  function getBirthDate(patient) {
    if (patient?.dateOfBirth !== undefined) return patient.dateOfBirth;
    return new Date(
      patient.yearOfBirth,
      (patient?.monthOfBirth ?? 1) - 1,
  ...
  return (
    <p>
      ...
      (Age:<strong> {!isNaN(age) ? age : "-"} </strong>, Gender:
      <strong> {getPatientGender(patient?.gender) || "-"}</strong>)<br />
      {address.length !== 0 && (
        <span>
          Address: {address}
          <br />
      ...
      Mobile:{" "}
      {patient?.phoneNumber ||
        (patient?.identifiers != null ? patient?.identifiers[0]?.value : "-")}
      {patient?.id !== undefined && (
        <span>
          <br />
          ABHA Address: {patient?.id}
        </span>
      )}
      ...
      {healthIdNumber !== undefined && healthIdNumber !== null && (
        <span>
          <br />
          ABHA Number: {healthIdNumber}
        </span>
      )}
    </p>
  );
